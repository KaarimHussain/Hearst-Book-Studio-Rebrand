...
// Get all non-fiction books (Business, Self-Help, Technology)
const nonFictionBooks: Book[] = BOOKS.filter(
  (b) => b.category === "Business" || b.category === "Self-Help" || b.category === "Technology"
);

function NonFictionCoverImage({ book }: { book: Book }) {
...
export default function NonFictionShowcase() {
  const [searchQuery, setSearchQuery] = useState("");
  const [selectedCategory, setSelectedCategory] = useState<string>("All");

  const categories = ["All", "Business", "Self-Help", "Technology"];

  const filteredBooks = useMemo(() => {
    return nonFictionBooks.filter((book) => {
    ...
    <div className="w-full">
      ...
      <div className="flex flex-col md:flex-row items-center justify-between gap-4 mb-8 bg-black/60 p-4 sm:p-5 border border-[#3075ba]/30">
        ...
        <div className="relative w-full md:w-72">
          ...
          <input
            ...
            placeholder="Search non-fiction titles, authors..."
      ...
      {filteredBooks.length > 0 ? (
            ...
            >
              <div>
                ...
                <div className="h-64 sm:h-72 overflow-hidden bg-black/40 p-4 flex items-center justify-center relative border-b border-white/10">
                  <NonFictionCoverImage book={item} />
        ...
        <div className="text-center py-12 px-4 bg-black/50 border border-white/10 text-gray-300 text-sm">
          No non-fiction titles found matching your search. Try resetting your query.
        </div>
      )}
    </div>
  );
