...
export interface BusinessBookItem extends Book {
  subCategory: "Leadership & Strategy" | "Startups & Innovation" | "Finance & Economics";
}

const businessBooks: BusinessBookItem[] = BOOKS.filter(
  ...
  subCategory: (idx % 3 === 0
    ...
    : idx % 3 === 1
    ? "Startups & Innovation"
    : "Finance & Economics") as BusinessBookItem["subCategory"],
}));

function BusinessCoverImage({ book }: { book: Book }) {
  const [hasError, setHasError] = useState(false);

  if (hasError) {
    return (
      <div className="w-36 h-48 sm:w-40 sm:h-54 bg-gradient-to-br from-slate-950 via-blue-950 to-neutral-900 p-4 shadow-md flex flex-col justify-between border border-[#3075ba]/30 text-left select-none relative overflow-hidden">
...
export default function BusinessShowcase() {
  ...
  const subCategories = ["All", "Leadership & Strategy", "Startups & Innovation", "Finance & Economics"];

  const filteredBooks = useMemo(() => {
    return businessBooks.filter((book) => {
    ...
    <div className="w-full">
      <div className="flex flex-col md:flex-row items-center justify-between gap-4 mb-8 bg-black/60 p-4 sm:p-5 border border-[#3075ba]/30">
        ...
        <div className="relative w-full md:w-72">
          ...
          <input
            ...
            placeholder="Search business titles, CEOs..."
      ...
      {filteredBooks.length > 0 ? (
        ...
        <div className="text-center py-12 px-4 bg-black/50 border border-white/10 text-gray-300 text-sm">
          No business titles found matching your search.
        </div>
      )}
    </div>
  );
