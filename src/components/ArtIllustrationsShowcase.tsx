...
export interface ArtBookItem extends Book {
  subCategory: "Concept Art" | "Children's Illustrations" | "Digital Art & Covers";
}

const artBooks: ArtBookItem[] = [
  {
    id: "p-38",
    ...
    category: "Fiction",
    subCategory: "Children's Illustrations",
    service: "Design",
    rating: 4.8,
    description: "Interactive animal artwork and illustration collection.",
    link: "#",
  },
  {
    id: "p-39",
    ...
    service: "Design",
    rating: 4.9,
    description: "Masterclass in sketch drawing techniques and character design.",
    link: "#",
  },
  ...BOOKS.slice(0, 6).map((b, idx) => ({
    ...b,
    subCategory: (idx % 3 === 0
      ? "Concept Art"
      : idx % 3 === 1
      ? "Children's Illustrations"
      : "Digital Art & Covers") as ArtBookItem["subCategory"],
  })),
];

function ArtCoverImage({ book }: { book: Book }) {
  const [hasError, setHasError] = useState(false);

  if (hasError) {
    return (
      <div className="w-36 h-48 sm:w-40 sm:h-54 bg-gradient-to-br from-purple-950 via-pink-950 to-neutral-900 p-4 shadow-md flex flex-col justify-between border border-[#3075ba]/30 text-left select-none relative overflow-hidden">
...
export default function ArtIllustrationsShowcase() {
  const [searchQuery, setSearchQuery] = useState("");
  const [selectedSubCategory, setSelectedSubCategory] = useState<string>("All");

  const subCategories = ["All", "Concept Art", "Children's Illustrations", "Digital Art & Covers"];

  const filteredBooks = useMemo(() => {
    return artBooks.filter((book) => {
    ...
    <div className="w-full">
      <div className="flex flex-col md:flex-row items-center justify-between gap-4 mb-8 bg-black/60 p-4 sm:p-5 border border-[#3075ba]/30">
        ...
        <div className="relative w-full md:w-72">
          ...
          <input
            ...
            placeholder="Search art books or illustrators..."
      ...
      {filteredBooks.length > 0 ? (
        ...
        <div className="text-center py-12 px-4 bg-black/50 border border-white/10 text-gray-300 text-sm">
          No art & illustration titles found matching your search.
        </div>
      )}
    </div>
  );
