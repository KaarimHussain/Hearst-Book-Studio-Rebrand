...
export interface SelfHelpBookItem extends Book {
  subCategory: "Personal Growth" | "Mindfulness & Wellness" | "Productivity & Success";
}

// Get all self-help books from dataset and assign sub-categories for real-time filtering
const selfHelpBooks: SelfHelpBookItem[] = BOOKS.filter(
  (b) => b.category === "Self-Help"
).map((b, idx) => ({
  ...b,
  subCategory:
    idx % 3 === 0
...
function SelfHelpCoverImage({ book }: { book: Book }) {
  const [hasError, setHasError] = useState(false);
  const amazonFallbackUrl = `https://images-na.ssl-images-amazon.com/images/P/${book.id}.01._SCLZZZZZZZ_SX500_.jpg`;

  if (hasError) {
    return (
      <div className="w-36 h-48 sm:w-40 sm:h-54 bg-gradient-to-br from-blue-950 via-sky-950 to-neutral-900 p-4 shadow-md flex flex-col justify-between border border-[#3075ba]/30 text-left select-none relative overflow-hidden">
...
export default function SelfHelpShowcase() {
  const [searchQuery, setSearchQuery] = useState("");
  const [selectedSubCategory, setSelectedSubCategory] = useState<string>("All");

  const subCategories = ["All", "Personal Growth", "Productivity & Success", "Mindfulness & Wellness"];

  const filteredBooks = useMemo(() => {
    return selfHelpBooks.filter((book) => {
    ...
    <div className="w-full">
      ...
      <div className="flex flex-col md:flex-row items-center justify-between gap-4 mb-8 bg-black/60 p-4 sm:p-5 border border-[#3075ba]/30">
        ...
        <div className="relative w-full md:w-72">
          ...
          <input
            ...
            placeholder="Search self-help titles, authors..."
      ...
      {filteredBooks.length > 0 ? (
        ...
        <div className="text-center py-12 px-4 bg-black/50 border border-white/10 text-gray-300 text-sm">
          No self-help titles found matching your search. Try resetting your query.
        </div>
      )}
    </div>
  );
