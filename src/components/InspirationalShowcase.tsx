...
export interface InspBookItem extends Book {
  subCategory: "Life Transformation" | "Motivational Stories" | "Daily Devotionals";
}

const inspBooks: InspBookItem[] = BOOKS.slice(0, 8).map((b, idx) => ({
  ...b,
  ...
  subCategory: (idx % 3 === 0
    ...
    : "Daily Devotionals") as InspBookItem["subCategory"],
}));

function InspCoverImage({ book }: { book: Book }) {
  const [hasError, setHasError] = useState(false);

  if (hasError) {
    return (
      <div className="w-36 h-48 sm:w-40 sm:h-54 bg-gradient-to-br from-amber-900 via-sky-950 to-neutral-900 p-4 shadow-md flex flex-col justify-between border border-[#3075ba]/30 text-left select-none relative overflow-hidden">
...
export default function InspirationalShowcase() {
  const [searchQuery, setSearchQuery] = useState("");
  const [selectedSubCategory, setSelectedSubCategory] = useState<string>("All");

  const subCategories = ["All", "Life Transformation", "Motivational Stories", "Daily Devotionals"];

  const filteredBooks = useMemo(() => {
    return inspBooks.filter((book) => {
    ...
    <div className="w-full">
      <div className="flex flex-col md:flex-row items-center justify-between gap-4 mb-8 bg-black/60 p-4 sm:p-5 border border-[#3075ba]/30">
        ...
        <div className="relative w-full md:w-72">
          ...
          <input
            ...
            placeholder="Search inspirational titles..."
      ...
      {filteredBooks.length > 0 ? (
        ...
        <div className="text-center py-12 px-4 bg-black/50 border border-white/10 text-gray-300 text-sm">
          No inspirational titles found matching your search.
        </div>
      )}
    </div>
  );
