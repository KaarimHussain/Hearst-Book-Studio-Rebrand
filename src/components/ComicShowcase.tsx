...
export interface ComicBookItem extends Book {
  subCategory: "Graphic Novels" | "Superhero & Action" | "Manga & Anime";
}

const comicBooks: ComicBookItem[] = [
  {
    id: "p-36",
    ...
    service: "Writing",
    rating: 4.9,
    description: "International bestseller graphic novel series.",
    link: "#",
  },
  {
    id: "p-40",
    ...
    author: "Dav Pilkey",
    category: "Fiction",
    subCategory: "Superhero & Action",
    service: "Publishing",
    rating: 4.9,
    description: "High-action comic graphic novel full of humor.",
    link: "#",
  },
  ...BOOKS.slice(0, 6).map((b, idx) => ({
    ...
    subCategory: (idx % 3 === 0
      ...
      : idx % 3 === 1
      ? "Superhero & Action"
      : "Manga & Anime") as ComicBookItem["subCategory"],
  })),
];

function ComicCoverImage({ book }: { book: Book }) {
  const [hasError, setHasError] = useState(false);

  if (hasError) {
    return (
      <div className="w-36 h-48 sm:w-40 sm:h-54 bg-gradient-to-br from-red-950 via-blue-950 to-neutral-900 p-4 shadow-md flex flex-col justify-between border border-[#3075ba]/30 text-left select-none relative overflow-hidden">
...
export default function ComicShowcase() {
  ...
  const subCategories = ["All", "Graphic Novels", "Superhero & Action", "Manga & Anime"];

  const filteredBooks = useMemo(() => {
    return comicBooks.filter((book) => {
    ...
    <div className="w-full">
      <div className="flex flex-col md:flex-row items-center justify-between gap-4 mb-8 bg-black/60 p-4 sm:p-5 border border-[#3075ba]/30">
        ...
        <div className="relative w-full md:w-72">
          ...
          <input
            ...
            placeholder="Search comic books or authors..."
      ...
      {filteredBooks.length > 0 ? (
        ...
        <div className="text-center py-12 px-4 bg-black/50 border border-white/10 text-gray-300 text-sm">
          No comic books found matching your search.
        </div>
      )}
    </div>
  );
